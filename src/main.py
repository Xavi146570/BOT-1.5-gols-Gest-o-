...
def schedule_next(interval_hours: float):
    logger.info(f"Próxima análise em {interval_hours} horas.")
    Timer(interval_hours * 3600, scheduled_cycle).start()
...
def scheduled_cycle():
    try:
        ...
        schedule_next(interval_hours)

if __name__ == "__main__":
    logger.info("✅ Scheduler inicializado com sucesso")
    # run initial cycle asynchronously to not block startup
    Timer(1, scheduled_cycle).start()
    # start uvicorn
